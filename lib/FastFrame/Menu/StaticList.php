    ...
    function _getMenuNode($in_data, $in_level)
    {
        ...
        foreach ($in_data as $s_key => $a_data) {
            if (is_int($s_key) && is_array($a_data)) {
                ...
                if (!$b_hitSub) {
                    $b_hitSub = true;
                    $tmp_html = $this->_processPerms($a_data, '<div style="float: right;">&#187;</div>');
                    $s_node = preg_replace('|<li>(<a.*?">)(.*?)</a>|', '<li class="parent">\\1<div style="cursor: pointer; float: left;">\\2</div>' . $tmp_html . '</a>', $s_node);
                }
